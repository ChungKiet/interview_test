...
class sendEmail():
   def __init__(self, templateJSONPath, customerFilePath, folderOutputPath, errorFilePath):
      self.templateJSONPath = templateJSONPath
      self.customerFilePath = customerFilePath
      self.folderOutputPath = folderOutputPath
   ...
   def send(self):
      ...
      df = pd.read_csv(self.customerFilePath)

      header = [h for h in df]
      numOfRecord = len(df[header[TITLE]])

      templateJSONFile = open(self.templateJSONPath)
      ...
      for i in range(numOfRecord):
         ...
         if len(email) > 0 and EMAIL_POST_FIX in email:
            ...
            with open(self.folderOutputPath  + df[header[EMAIL]][i] + '.json', 'w', encoding='UTF8', newline='') as result:
      ...
      with open(self.errorFilePath, 'w', encoding='UTF8', newline='') as errorFile:
